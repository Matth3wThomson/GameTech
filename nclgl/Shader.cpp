...
Shader::Shader(string vFile, string fFile, string gFile){
	...
	//Create a pointer to a gl shader
	program		= glCreateProgram();

	//Generate pointers to the shader objects, and compile them from the files
	...
	//If a geometry shader file location was supplied, do the same for the geom shader
	if(!gFile.empty()) {
...
Shader::~Shader(void)	{
	//Detach all of the shader objects, and delete the objects
	for(int i = 0; i < 3; ++i) {
